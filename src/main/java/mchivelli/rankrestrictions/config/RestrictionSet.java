...
public class RestrictionSet {
    ...
    private final Set<String> restrictedModIds = new HashSet<>(); // Store just the mod ID, e.g., "mekanism"
    private final List<TagKey<Item>> restrictedTags = new ArrayList<>();
    ...
    public RestrictionSet(List<String> rawItemPatterns, String message) {
        ...
        preprocessRestrictions(rawItemPatterns);
    }

    private void preprocessRestrictions(List<String> patterns) {
        for (String patternGroup : patterns) {
            ...
            for (String p : individualPatterns) {
                ...
                if (trimmedPattern.startsWith("#")) { // Tag
                    ...
                    if (tagId != null) {
                        restrictedTags.add(TagKey.create(ForgeRegistries.ITEMS.getRegistryKey(), tagId));
                    } else {
                    ...
                    restrictedModIds.add(trimmedPattern.substring(0, trimmedPattern.length() - 2));
                } else { // Exact item ID
    ...
    public boolean equals(Object o) {
        ...
        return restrictedExactItems.equals(that.restrictedExactItems) &&
               restrictedModIds.equals(that.restrictedModIds) &&
               restrictedTags.equals(that.restrictedTags) &&
               Objects.equals(message, that.message);
    }

    @Override
    public int hashCode() {
        return Objects.hash(restrictedExactItems, restrictedModIds, restrictedTags, message);
    }
    ...
    public boolean isRestricted(ResourceLocation itemRL, Item itemToCheck) {
        ...
        if (restrictedModIds.contains(itemRL.getNamespace())) {
            return true;
        }
        ...
        for (TagKey<Item> tagKey : restrictedTags) {
